...
| LearningOS UI | Module/unit/stage presentation and guarded actions. | Reads manifest contract v9 (`src/contracts/manifest.ts`'s `MANIFEST_CONTRACT_VERSION`) only, refusing any other contract version before exposing data; bundled output follows Obsidian custom-view/command/settings conventions. |
