...
async function main() {
  ...
  heading('navigation and boundaries');
  {
    ...
    check('rail matches the product model', ['Bachelor’s', 'Skills', 'Thesis & projects', 'Shelving', 'Library', 'Inbox', 'Master’s', 'Job']
  ...
  heading('module and component ownership');
  {
    ...
    view.contentEl.findText('los-btn', 'Analysis').fire('click');
    text = view.contentEl.allText();
    check('component selection filters units without merging state', text.includes('Analysis exam prep') && !text.includes('Lecture 04'));
  ...
  }
  {
    ...
    for (const [label, pattern] of [
      ...
      ['legacy global-path commands', /path-note|path-progress|openPath\(/],
    ...
    check('bundle is generated from modular TypeScript', fs.readdirSync(path.join(ROOT, 'src', 'views')).length >= 8
